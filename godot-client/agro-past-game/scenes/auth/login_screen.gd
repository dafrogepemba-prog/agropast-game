...
btn_mtn.modulate = Color.WHITE if op == ""MTN"" else Color(1, 1, 1, 0.5)
btn_airtel.modulate = Color.WHITE if op == ""AIRTEL"" else Color(1, 1, 1, 0.5)

func _on_connect_pressed() -> void:
error_label.text = """"

if phone_input.text.length() < 9:
error_label.text = ""Numéro invalide""
return
...
# Afficher section OTP
otp_section.visible = true
btn_connect.text = ""VÉRIFIER""
...
error_label.text = ""Code vérifié !""  # TODO: appel AuthManager
else:
